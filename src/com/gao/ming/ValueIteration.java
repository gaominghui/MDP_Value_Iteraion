	...
	public int solve (){
		...
		while(!finished) {
			...
			for(State state=mdp.getStartState();state!=null;state=mdp.getNextState()) {
				...
				//System.out.println(state.toString()+"$$$$"+utility+"$$$$"+reward+"$$$$");
				...
				for(Action action=mdp.getStartAction(); action!=null; 
					...
					for(int i=0; i<s; ++i) {
						...
						//prob+"%%%%"+sPrime+mdp.getUtility(sPrime)));
			...
			///	System.out.println(state.toString()+"$$$$"+utility+"$$$$"+reward
			//			+"$$$$"+maxAction.toString()+"$$$$"+maxCurrentUtil);				
				double currentError=Math.abs(maxCurrentUtil-utility);
				if(currentError>maxError)
					maxError = currentError;
				System.out.println(maxCurrentUtil+"%%%%"+utility+"%%%%"+maxError+"%%%%"+currentError);
			}
