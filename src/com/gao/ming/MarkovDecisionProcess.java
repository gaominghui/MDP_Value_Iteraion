...
public class MarkovDecisionProcess {
	...
	public void punchHole(int row, int col){
		// dangling references will be collected by the GC
	...
	public void compileStates() {
		...
		// A list of probability-state pairs.  See Transition.java
	...
	public Vector getTransition(State s, Action a) {
		...
		for(int i=0; i<numResultingStates; ++i) {
			...
			// i is also the kind of action needs to be taken to
			// get to the next state.
			// nextState = grid[rel2absolute[i][0]][rel2absolute[i][1]]
			// won't work, because we have to consider boundaries.  Whereas
			// the method move() takes care of that.
			
			// Now, that i, which refers to resulting states, happens to 
			// be action types as well, is a very unfortunate design.  It
			// makes the method move() to do extra work (STAY).
			// A slightly better way to call move() is move(s, actions[i].action)
			// or move(s, actions[i]) if we define an overloaded move().
			nextState = move(s, i);
	...
	public State transit(State s, Action a) {
		...
		// Create the distribution of T(s,a,s')
		...
		// Pull out a value (probability) from a unifrom distribution
		// Hopefully.
		double p = Math.random();
		int next=0;
		if(p<dist[0]) {
			// Next state is the one UP
			next = 0;
		}else {
			for(int i=1; i<numResultingStates; ++i){
		...
		// Don't have to use move() for the bounced-back situations
		// It's all in the transition model already.
